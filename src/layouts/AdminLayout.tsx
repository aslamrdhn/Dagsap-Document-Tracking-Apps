    ...
    <div className="h-[100dvh] w-full bg-slate-50 flex font-sans overflow-hidden relative">
      ...
      `}>
        ...
        <nav className="flex-1 py-6 px-4 space-y-1 overflow-y-auto">
          ...
          <Link to="/admin/documents" className={getLinkClass("/admin/documents")} onClick={closeSidebar}>
            <FileText size={20} className="mr-3 shrink-0" />
            <span className="text-sm font-medium">Documents</span>
          </Link>
          <Link to="/admin/locations" className={getLinkClass("/admin/locations")} onClick={closeSidebar}>
          ...
          <div className="flex items-center justify-between mb-4"> 
            <div className="flex items-center space-x-3 overflow-hidden pr-2">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-white text-xs font-semibold truncate">{user?.name || 'Admin'}</span>
                <span className="text-white/50 text-[10px] truncate">{user?.role}</span>
              </div>
            </div>
            <button onClick={onLogout} className="text-white/60 hover:text-white transition-colors shrink-0 p-2" aria-label="Logout">
              <LogOut size={16} />
            </button>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col h-full min-w-0 bg-slate-50 overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 px-4 lg:px-8 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center space-x-3 lg:space-x-6 shrink-0">
            ...
            <Link to="/admin/documents" className="bg-[#800000] hover:bg-[#600000] transition-colors text-white px-3 lg:px-4 py-2 rounded-lg text-sm font-semibold shadow-md flex items-center">
