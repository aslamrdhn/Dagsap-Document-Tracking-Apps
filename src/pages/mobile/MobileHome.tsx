      ...
      <motion.div variants={itemVariants}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden divide-y divide-slate-50">
          {stats.recentDocs.length === 0 ? (
             <div className="p-8 text-sm text-slate-400 text-center font-medium">No recent documents</div>
          ) : (
            stats.recentDocs.map((doc: any) => (
              <div key={doc.id} className="p-4 flex items-center justify-between active:bg-slate-100 transition-colors">
                <div>
                  <div className="font-mono font-bold text-[#800000] text-sm">{doc.documentNumber}</div>
