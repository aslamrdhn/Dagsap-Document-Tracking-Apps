              ...
              {documents.length === 0 ? (
                ...
                <tr key={doc.id} className="hover:bg-slate-50">
                  ...
                  <td className="p-4 text-slate-600 text-xs">
                    <span className="font-semibold">{doc.originLocation?.code}</span> → <span className="font-semibold">{doc.destinationLocation?.code}</span>
                  </td>
                  <td className="p-4">
                    <span className="px-2 py-1 bg-slate-100 text-slate-700 rounded-md text-[10px] font-bold">{doc.status}</span>
                  </td>
                  <td className="p-4">
                    <button 
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Create New Document">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select required value={formData.documentTypeId} onChange={e => setFormData({...formData, documentTypeId: e.target.value})} className="w-full p-2 border rounded-lg text-sm bg-slate-50">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select required value={formData.originLocationId} onChange={e => setFormData({...formData, originLocationId: e.target.value})} className="w-full p-2 border rounded-lg text-sm bg-slate-50">
            ...
            <div>
              ...
              <select required value={formData.destinationLocationId} onChange={e => setFormData({...formData, destinationLocationId: e.target.value})} className="w-full p-2 border rounded-lg text-sm bg-slate-50">
          ...
          <div>
            ...
            <select value={formData.priority} onChange={e => setFormData({...formData, priority: e.target.value})} className="w-full p-2 border rounded-lg text-sm bg-slate-50">
          ...
          <div>
            ...
            <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full p-2 border rounded-lg text-sm bg-slate-50 h-20" placeholder="Enter notes..."></textarea>
          </div>
          <button type="submit" className="w-full bg-[#800000] text-white p-3 rounded-lg font-bold shadow hover:bg-[#600000] transition">Create Document</button>
        </form>
      </Modal>
