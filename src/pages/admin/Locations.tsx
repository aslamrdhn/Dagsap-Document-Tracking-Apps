...
export default function Locations() {
  ...
  if (loading) return <div className="p-8 text-[#888888] font-medium">Loading locations interface...</div>;

  return (
    <div className="p-4 lg:p-8 max-w-[1600px] mx-auto h-full flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
        <div>
          <h1 className="text-2xl font-black text-[#111111] tracking-tight">Facilities & Transit Points</h1>
          <p className="text-[#666666] text-sm mt-1 font-medium">Manage operational nodes and physical branches.</p>
        </div>
        <button 
      ...
      <div className="bg-white border border-[#E5E5E4] rounded-xl shadow-sm flex flex-col overflow-hidden flex-1">
        ...
        <div className="p-4 border-b border-[#E5E5E4] bg-[#F7F7F5] flex justify-between items-center shrink-0">
          <div className="flex items-center bg-white border border-[#E5E5E4] rounded-md px-3 py-1.5 w-64 focus-within:border-[#111111] transition-colors">
            ...
            <input type="text" placeholder="Filter locations..." className="bg-transparent border-none outline-none text-xs text-[#111111] w-full font-medium" />
          </div>
          <div className="text-[10px] font-bold text-[#888888] uppercase tracking-widest">{locations.length} Locations</div>
        </div>
        ...
        <div className="overflow-auto flex-1 bg-white">
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead className="bg-white sticky top-0 z-10 shadow-[0_1px_0_0_#E5E5E4]">
              <tr>
                <th className="p-4 text-[10px] font-bold text-[#666666] uppercase tracking-[0.15em] border-b border-[#E5E5E4]">Code</th>
                <th className="p-4 text-[10px] font-bold text-[#666666] uppercase tracking-[0.15em] border-b border-[#E5E5E4]">Name</th>
                <th className="p-4 text-[10px] font-bold text-[#666666] uppercase tracking-[0.15em] border-b border-[#E5E5E4]">Type</th>
                <th className="p-4 text-[10px] font-bold text-[#666666] uppercase tracking-[0.15em] border-b border-[#E5E5E4] text-center">Status</th>
            ...
            <tbody className="divide-y divide-[#E5E5E4] text-sm">
              {locations.length === 0 ? (
                <tr><td colSpan={5} className="p-10 text-center text-[#A0A0A0] font-medium">No locations found.</td></tr>
              ) : locations.map(loc => (
                <tr key={loc.id} className="hover:bg-[#F9F9F8] transition-colors group">
                  ...
                  <td className="p-4 text-center">
                    <span className={`text-[10px] font-black uppercase tracking-widest ${loc.active ? "text-[#10B981]" : "text-[#EF4444]"}`}>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={isEditMode ? "Edit Location" : "Add Location"}>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-[10px] font-bold text-[#666666] uppercase tracking-widest mb-1.5">Code</label>
            <input 
              required 
              maxLength={5} 
              value={formData.code} 
          ...
          <div>
            <label className="block text-[10px] font-bold text-[#666666] uppercase tracking-widest mb-1.5">Name</label>
            <input 
              required 
              value={formData.name} 
          ...
          <div>
            <label className="block text-[10px] font-bold text-[#666666] uppercase tracking-widest mb-1.5">Type</label>
            <select 
              required 
              value={formData.type} 
