    ...
    <div className="flex flex-col min-h-[calc(100vh-4rem)] bg-slate-50">
      <section className="p-4 lg:p-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 shrink-0">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
          <div className="text-slate-400 text-[10px] font-bold uppercase tracking-wider mb-1">Total Documents</div>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
          <div className="text-[#800000] text-[10px] font-bold uppercase tracking-wider mb-1">In Transit</div>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
          <div className="text-orange-500 text-[10px] font-bold uppercase tracking-wider mb-1">At Transit Points</div>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
          <div className="text-red-600 text-[10px] font-bold uppercase tracking-wider mb-1">Overdue SLA</div>
      ...
      <section className="px-4 lg:px-8 pb-4 lg:pb-8 flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 min-h-0">
        <div className="col-span-1 lg:col-span-8 bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden min-h-[400px]">
          <div className="p-5 border-b flex justify-between items-center bg-white shrink-0">
            <h2 className="font-bold text-slate-800 text-sm md:text-base">Live Monitoring: Active Chain of Custody</h2>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden md:inline">Auto-refresh in 4s</span>
          </div>
          <div className="flex-1 overflow-auto p-0">
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead className="bg-slate-50 sticky top-0 z-10 shadow-[0_1px_0_0_#e2e8f0]">
                <tr>
                  <th className="p-4 text-[11px] font-bold text-slate-500 uppercase border-b whitespace-nowrap">Doc ID</th>
                  <th className="p-4 text-[11px] font-bold text-slate-500 uppercase border-b whitespace-nowrap">Current Location</th>
                  <th className="p-4 text-[11px] font-bold text-slate-500 uppercase border-b whitespace-nowrap">Holder</th>
                  <th className="p-4 text-[11px] font-bold text-slate-500 uppercase border-b whitespace-nowrap">Status</th>
                  <th className="p-4 text-[11px] font-bold text-slate-500 uppercase border-b whitespace-nowrap">Last Update</th>
                </tr>
              </thead>
              <tbody className="text-sm divide-y">
                {activeDocs.length === 0 ? (
                  <tr><td colSpan={5} className="p-8 text-center text-slate-400">No active documents right now.</td></tr>
                ) : activeDocs.map((doc: any) => (
                  <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4 font-mono font-bold text-[#800000] whitespace-nowrap">{doc.documentNumber}</td>
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded-md text-[10px] font-bold whitespace-nowrap ${
                        doc.status === 'IN_TRANSIT' ? 'bg-orange-100 text-orange-700' :
          ...
          <div className="bg-[#1a1c23] rounded-2xl p-6 text-white shrink-0">
            ...
            <div className="border-2 border-dashed border-white/20 rounded-xl h-24 flex items-center justify-center bg-white/5 cursor-pointer hover:bg-white/10 transition-colors">
              <div className="flex flex-col items-center">
                <QrCode className="w-6 h-6 text-white/40 mb-2" />
                <span className="text-xs font-medium text-white/80">Upload Label Image</span>
              </div>
            </div>
            <div className="mt-4 flex justify-between items-center text-[11px]">
              <span className="text-white/40">Supports QR</span>
