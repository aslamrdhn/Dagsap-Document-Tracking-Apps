...
router.get('/dashboard', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (user.role === 'COURIER') {
      inTransit = await prisma.document.count({
        where: { status: 'IN_TRANSIT', currentHolder: user.name },
      });
      ...
      inTransit = await prisma.document.count({
        where: { destinationLocationId: user.defaultLocationId, status: 'IN_TRANSIT' },
      });
      completed = await prisma.document.count({
        where: { destinationLocationId: user.defaultLocationId, status: 'COMPLETED' },
      });
      recentDocs = await prisma.document.findMany({
        where: {
          OR: [
            { destinationLocationId: user.defaultLocationId },
            { currentLocationId: user.defaultLocationId },
          ],
        },
        orderBy: { updatedAt: 'desc' },
        take: 5,
